...
class FlsDetection(object):
    ...
    def pub_projected_img(self, ranges, thetas):
        ...
        camera_hfov = np.arctan((self.camera_info.width/2)/self.camera_info.K[0])
        h   = int(self.camera_info.height * self.scale_factor)
        w   = int(self.camera_info.width   * self.scale_factor )
        fx  = int(self.camera_info.K[0]  * self.scale_factor)
        fy  = int(self.camera_info.K[4]  * self.scale_factor)
        cx  = int(self.camera_info.K[2]  * self.scale_factor)
        cy  = int(self.camera_info.K[5]  * self.scale_factor)
    ...
    def fuse_camera_and_sonar(self, camera_mask, sonar_mask):
        #print("camera_mask", type(camera_mask), camera_mask)
        ...
        if sonar_mask.size == camera_mask.size:
            ...
            fx  = int(self.camera_info.K[0]  * self.scale_factor)
            fy  = int(self.camera_info.K[4]  * self.scale_factor)
            cx  = int(fused_image_pub.shape[1]/2  * self.scale_factor) #self.camera_info.K[2] 
            cy  = int(fused_image_pub.shape[0]/2   * self.scale_factor)#self.camera_info.K[5] 
